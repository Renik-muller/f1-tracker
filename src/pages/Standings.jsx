                        ...
                        <div className="chart-container card">
                            <ResponsiveContainer width="100%" height={280}>
                                <BarChart data={chartData} margin={{ top: 10, right: 20, left: 0, bottom: 60 }}>
                                    <XAxis dataKey="name" tick={{ fill: '#888', fontSize: 11, fontFamily: 'Inter' }} angle={-40} textAnchor="end" interval={0} />
                            ...
                            <table className="f1-table">
                                <thead>
                                    <tr>
                                        <th>#</th>
                                        {tab === 'drivers' ? (
                                            <><th>Driver</th><th>Team</th><th>Wins</th><th>Points</th></>
                                        ) : (
                                    ...
                                    {tab === 'drivers'
                                            ...
                                            <tr key={s.Driver.driverId}>
                                                ...
                                                <td>
                                                    <div className="team-cell">
                                                        ...
                                                        <span style={{ color: getTeamColor(s.Constructors?.[0]?.constructorId) }}>{s.Constructors?.[0]?.name}</span>
                                                    </div>
                                                </td>
                                                <td>{s.wins}</td>
                                        ...
                                        : (ctors || []).map((s, i) => (
                                            <tr key={s.Constructor.constructorId}>
                                                <td><span className={i < 3 ? `pos-${i + 1}` : ''}>{s.position}</span></td>
                                                <td style={{ color: getTeamColor(s.Constructor.constructorId), fontWeight: 600 }}>{s.Constructor.name}</td>
